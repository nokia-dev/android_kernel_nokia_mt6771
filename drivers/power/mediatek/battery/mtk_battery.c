				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		...
		val->intval = FG_status.soc * 3000 * 1000 / 100;//change to uAh
	...
	case POWER_SUPPLY_PROP_batt_temp:
		if(fih_get_fake_temp() == 999)
			...
			val->intval = fih_get_fake_temp() * 10;
		break;
...
/* Internal function */
/* ============================================================ */
int g_fg_battery_id = FIH_BATT_ID_UNKNOWN;
static char *manufacture;
...
void fgauge_get_profile_id(void)
{
	...
	if (ret != 0)
		bm_err("[fgauge_get_profile_id]id_volt read fail\n");
	else
	...
	for (id = 0; id < TOTAL_BATTERY_NUMBER; id++) {
		if((id_volt >= g_battery_id_min_voltage[id]) && (id_volt <= g_battery_id_max_voltage[id]))
		{
			g_fg_battery_id = id + 1;
			break;
		}
		else
			g_fg_battery_id = 0;
...
char *get_battery_id(void)
{
	if(g_fg_battery_id == FIH_BATT_ID_ATL)
		manufacture = "ATL";
	else if(g_fg_battery_id == FIH_BATT_ID_COSLIGHT)
		manufacture = "COSLIGHT";
	else if(g_fg_battery_id == FIH_BATT_ID_BAK)
		manufacture = "BAK";
	else
		manufacture = "UNKNOWN";
	return manufacture;
