...
extern int tpd_type_cap;              //-> Check if assemble LCM & Touch
extern int tpd_usb_status;            //-> Keep charger status info for Touch
extern uint8_t bTouchIsAwake;         //-> Check Touch is active or sleep
extern int tpd_usb_plugin(int plugin);//-> Configure touch firmware in AC mode or normal mode
// for touch charging flag ----------------------------------------------------- ed.

static int fih_fake_temp = 999;
extern int g_fg_battery_id;

int fih_get_fake_temp(void)
{
	return fih_fake_temp;
}
...
// for touch charging flag ----------------------------------------- st.
	if (tpd_type_cap) {
  // Only change firmware mode when to assemble with LCM & Touch
	   if (bTouchIsAwake) {
	   	   // Touch is active, confgure touch firmware directly
	       tpd_usb_plugin(USB_PLUGIN);
	   }
	   tpd_usb_status = USB_PLUGIN;
	   // Keep charger status, touch resume will configure fw mode based this info
  }
// for touch charging flag ----------------------------------------- ed.
...
extern int fih_check_temp(void);
...
static void charger_check_status(struct charger_manager *info)
{
	...
	if(g_fg_battery_id == FIH_BATT_ID_UNKNOWN)
	{
		charging = false;
		goto stop_charging;
	}
...
stop_charging:
	...
	chr_err("tmp:%d (jeita:%d sm:%d cv:%d en:%d) (sm:%d) en:%d c:%d s:%d ov:%d %d %d g_fg_battery_id:%d\n",
		temperature, info->enable_sw_jeita, info->sw_jeita.sm,
		info->sw_jeita.cv, info->sw_jeita.charging, thermal->sm,
		charging, info->cmd_discharging, info->safety_timeout,
		info->vbusov_stat,
		info->can_charging, charging, g_fg_battery_id);
...
static int mtk_charger_parse_dt(struct charger_manager *info, struct device *dev)
{
	...
	info->en_fih_pd_current = of_property_read_bool(np, "en_fih_pd_current");
	...
	/* OEM jeita*/
	info->en_fih_jeita = of_property_read_bool(np, "en_fih_jeita");
	if (of_property_read_u32(np, "fih_fv_h", &val) >= 0) {
		info->fih_fv_h = val;
	}
	if (of_property_read_u32(np, "fih_fv_l", &val) >= 0) {
		info->fih_fv_l = val;
	}
	if (of_property_read_u32(np, "fih_fcc_h", &val) >= 0) {
		info->fih_fcc_h = val;
	}
	if (of_property_read_u32(np, "fih_fcc_l", &val) >= 0) {
		info->fih_fcc_l = val;
	}
	if (of_property_read_u32(np, "fih_hot", &val) >= 0) {
		info->fih_hot = val;
	}
	if (of_property_read_u32(np, "fih_warm", &val) >= 0) {
		info->fih_warm = val;
	}
	if (of_property_read_u32(np, "fih_cool", &val) >= 0) {
		info->fih_cool = val;
	}
	if (of_property_read_u32(np, "fih_cold", &val) >= 0) {
		info->fih_cold = val;
	}

	/* OEM step charging*/
	info->en_fih_step_charging = of_property_read_bool(np, "en_fih_step_charging");
	if (of_property_read_u32(np, "fih_step_vol_h", &val) >= 0) {
		info->fih_step_vol_h = val;
	}
	if (of_property_read_u32(np, "fih_step_vol_l", &val) >= 0) {
		info->fih_step_vol_l = val;
	}
	if (of_property_read_u32(np, "fih_step_cur_h", &val) >= 0) {
		info->fih_step_cur_h = val;
	}
	if (of_property_read_u32(np, "fih_step_cur_l", &val) >= 0) {
		info->fih_step_cur_l = val;
	}

	/* OEM typec current */
	info->en_fih_typec_current = of_property_read_bool(np, "en_fih_typec_current");
	if (of_property_read_u32(np, "fih_typec_cable_c", &val) >= 0) {
		info->fih_typec_cable_c = val;
	}
	if (of_property_read_u32(np, "fih_typec_bat_c", &val) >= 0) {
		info->fih_typec_bat_c = val;
	}
...
static ssize_t show_Batt_ID_Volt(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	int ret = 0;
	int id_volt = 0;

	ret = IMM_GetOneChannelValue_Cali(2, &id_volt);

	return sprintf(buf, "%u\n", id_volt);
}

static DEVICE_ATTR(Batt_ID_Volt, 0444, show_Batt_ID_Volt, NULL);
...
static ssize_t show_Charger_Online(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	unsigned char exist;
...
static DEVICE_ATTR(Charger_Online, 0444, show_Charger_Online, NULL);

static ssize_t store_fih_Input_Current(struct device *dev, struct device_attribute *attr, const char *buf,
				 size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	unsigned int reg = 0;
	...
	if (buf != NULL && size != 0) {
		...
		printk("%s reg=%d\n", __func__,reg);
		pinfo->data.ac_charger_input_current = reg;
		_wake_up_charger(pinfo);
	}

	return size;
}
static DEVICE_ATTR(fih_Input_Current, 0644, NULL, store_fih_Input_Current);

static ssize_t store_fih_Charging_Control(struct device *dev, struct device_attribute *attr, const char *buf,
				 size_t size)
{
	struct charger_manager *pinfo = dev->driver_data;
	unsigned int reg = 0;
...
static ssize_t show_fih_Charging_Control(struct device *dev, struct device_attribute *attr,
					char *buf)
{
	bool result = false;
	struct charger_manager *pinfo = dev->driver_data;

	charger_dev_is_charging_enabled(pinfo->chg1_dev, &result);

	return sprintf(buf, "%u\n", result);
}
static DEVICE_ATTR(fih_Charging_Control, 0644, show_fih_Charging_Control, store_fih_Charging_Control);

static ssize_t show_fih_Fake_Temp(struct device *dev, struct device_attribute *attr,
						  char *buf)
{
	return sprintf(buf, "%d\n", fih_fake_temp);
}

static ssize_t store_fih_Fake_Temp(struct device *dev,
						   struct device_attribute *attr, const char *buf,
						   size_t size)
{
	sscanf(buf, "%d", &fih_fake_temp);

	return size;
}

static DEVICE_ATTR(fih_Fake_Temp, 0664, show_fih_Fake_Temp,
		   store_fih_Fake_Temp);
						...
						size_t count, loff_t *data)
{
	...
	if (sscanf(desc, "%d %d", &cmd_current_unlimited, &cmd_discharging) == 2) {
		...
		if (cmd_discharging == 1) {
			...
			if(info->polling_interval != CHARGING_INTERVAL)
	...
	size_t count, loff_t *data)
{
	...
	if (ret == 0) {
		...
		if(mtk_is_charger_on(info))
		{
			...
			pr_err("%s: NOT PLUG ANY CHARGER!\n", __func__);
		pr_err("%s: enable power path = %d\n", __func__, enable);
		return count;
	}
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_Batt_ID_Volt);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_fih_Input_Current);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_fih_Charging_Control);
	if (ret)
		goto _out;

	ret = device_create_file(&(pdev->dev), &dev_attr_fih_Fake_Temp);
	if (ret)
		goto _out;
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_Charger_Online);
	if (ret)
		goto _out;
