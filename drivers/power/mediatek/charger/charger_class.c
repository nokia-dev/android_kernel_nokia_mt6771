...
int charger_dev_is_charging_enabled(struct charger_device *charger_dev, bool *en)
{
	if (charger_dev != NULL && charger_dev->ops != NULL && charger_dev->ops->is_charging_enabled)
		return charger_dev->ops->is_charging_enabled(charger_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_is_charging_enabled);
