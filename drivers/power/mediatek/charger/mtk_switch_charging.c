...
int fih_check_temp(void)
{
	if(fih_get_fake_temp() == 999)
		return battery_get_bat_temperature();
	else
		return fih_get_fake_temp();
}

static void fih_dynamic_set_fcc(struct charger_manager *info)
{
	int temp, vol;
	u32 ichg_main;
	...
	temp = fih_check_temp();
	vol = battery_get_bat_voltage();
	vol *= 1000;

	charger_dev_get_charging_current(info->chg1_dev, &ichg_main);

	if(temp >= info->fih_warm)
		pdata->charging_current_limit = info->fih_fcc_h;
	else if((temp > info->fih_cool) && (temp < info->fih_warm))
	{
		if(vol >= info->fih_step_vol_h)
			pdata->charging_current_limit = info->fih_step_cur_l;
		else if(vol <= info->fih_step_vol_l)
			pdata->charging_current_limit = info->fih_step_cur_h;
	}
	else if(temp <= info->fih_cool)
		pdata->charging_current_limit = info->fih_fcc_l;
}
...
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (mtk_is_TA_support_pd_pps(info)) {
		...
		if (tcpm_inquire_typec_remote_rp_curr(info->tcpc) == 3000) {
			if(info->en_fih_typec_current)
			{
				pdata->input_current_limit = info->fih_typec_cable_c;
				pdata->charging_current_limit = info->fih_typec_bat_c;
			}
			else
			{
	...
	if(info->en_fih_jeita)
		fih_dynamic_set_fcc(info);
...
static void fih_dynamic_set_fv(struct charger_manager *info)
{
	int temp;
	u32 fv, new_fv = 0;

	temp = fih_check_temp();
	charger_dev_get_constant_voltage(info->chg1_dev, &fv);

	if((temp >= info->fih_warm))
	{
		if(fv != info->fih_fv_h)
			new_fv = info->fih_fv_h;
		else
			return;
	}
	else
	{
		if(fv != info->fih_fv_l)
			new_fv = info->fih_fv_l;
		else
			return;
	}

	charger_dev_set_constant_voltage(info->chg1_dev, new_fv);
}
