...
#ifndef LINUX_ZFS_SPI_REGS_H
#define LINUX_ZFS_SPI_REGS_H

#define ZFS1096_EXIT_TO_IDEL 		0x00
#define ZFS1096_DROP_INT_EVENT 		0x01

typedef enum {
	/* --- Common registers --- */
	ZFS1096_REG_READ_INTERRUPT_WITH_CLEAR			= 28,	/* 0x1C, RO, 2 bytes */
	ZFS1096_REG_READ_STATUS							= 32,	/* 0x20, RO, 2 byte	 */	
	ZFS1096_REG_READ_INTERRUPT						= 36,	/* 0x24, RO, 2 byte	 */
	ZFS1096_REG_READ_KEY_INTERRUPT					= 37,	/* 0x25, RW, 2 byte	 */
	ZFS1096_REG_READ_HW_INFO						= 178,	/* 0xB2, RW, 2 byte  */		
	ZFS1096_REG_READ_FINGER_STABLE					= 197,	/* 0xC5, RO, 3 byte	 */		
	ZFS1096_REG_READ_STABLE							= 212,	/* 0xD4, RO, 3 byte	 */	
	ZFS1096_REG_READ_KEY_STABLE						= 213,	/* 0xD5, RO, 3 byte	 */		
	ZFS1096_REG_MCU_RESET							= 228,	/* 0xE4, RW, 2 byte	 */		
} zfs_reg_t;

#define ZFS1096_REG_MAX_SIZE	10

#define ZFS1096_REG_SIZE(reg) (	\
	((reg) == ZFS1096_REG_READ_INTERRUPT_WITH_CLEAR) ?			2 : \
	((reg) == ZFS1096_REG_READ_INTERRUPT) ?						2 : \
	((reg) == ZFS1096_REG_READ_KEY_INTERRUPT) ?					2 : \
	((reg) == ZFS1096_REG_READ_STATUS) ?						2 : \
    ((reg) == ZFS1096_REG_READ_STABLE) ?						4 : \
	((reg) == ZFS1096_REG_READ_KEY_STABLE) ?					4 : \
	((reg) == ZFS1096_REG_MCU_RESET) ?							2 : \
	((reg) == ZFS1096_REG_READ_HW_INFO) ?						2 : \
	0)
