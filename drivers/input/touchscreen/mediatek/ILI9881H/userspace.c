...
//need midify fih_ili_selftest_open and ilitek_proc_mp_test_read
void fih_ili_selftest_open(void)
{
	//int apk[100] = {0};
	//uint32_t res;
...
static ssize_t ilitek_proc_fw_upgrade_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	if (*pPos != 0)
		return 0;
	temp_path = kmalloc(size * sizeof(uint8_t), GFP_KERNEL);
	memset(temp_path, 0, size);
	if (buff != NULL) {
		res = copy_from_user(temp_path, buff, size - 1);
		if (res < 0) {
			ipio_info("copy data from user space, failed\n");
			return -1;
		}
	...
	res = core_firmware_upgrade(temp_path, false);
...
static int netlink_init(void)
{
	...
	return res;  //20180814 add to skip 
