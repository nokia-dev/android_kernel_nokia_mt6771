...
static int32_t Ili_TestResult = 666;
...
static void mp_show_result(void)
{
	...
	if (pass_item_count == 0) {
		...
		Ili_TestResult=core_mp->final_result;
...
static void mp_run_test(char *item, bool ini)
{
	int i = 0;
	char str[512] = { 0 };
...
void core_mp_start_test(void)
{
	...
	if (protocol->major >= 5 && protocol->mid >= 4) {
		...
		mp_run_test("Noise Peak To Peak(With Panel)", true);
		mp_run_test("Noise Peak to Peak(IC Only)", true);
		mp_run_test("Short Test -ILI9881", true);
		mp_run_test("Open Test(integration)_SP", true);
		mp_run_test("Raw Data(Have BK)", true);
		/*mp_run_test("Raw Data(Have BK) (LCM OFF)", true);*/
		mp_run_test("Calibration Data(DAC)", true);
		mp_run_test("Raw Data(No BK)", true);
		mp_run_test("Raw Data(No BK) (LCM OFF)", true);
		mp_run_test("Noise Peak to Peak(With Panel) (LCM OFF)", true);
		/*mp_run_test("Noise Peak to Peak(IC Only) (LCM OFF)", true);*/
		mp_run_test("Raw Data_TD (LCM OFF)", true);
		mp_run_test("Peak To Peak_TD (LCM OFF)", true);
		mp_run_test("Doze Raw Data", true);
		mp_run_test("Doze Peak To Peak", true);

	} else {
		mp_run_test("Untouch Peak to Peak", true);
		mp_run_test("Open Test(integration)", true);
		mp_run_test("Open Test(Cap)", true);
		mp_run_test("Short Test (Rx)", true);
		mp_run_test("Untouch Calibration Data(DAC) - Mutual", true);
		mp_run_test("Untouch Raw Data(Have BK) - Mutual", true);
		mp_run_test("Untouch Raw Data(No BK) - Mutual", true);
		mp_run_test("Untouch Cm Data", true);
		mp_run_test("Pixel Raw (No BK)", true);
		mp_run_test("Pixel Raw (Have BK)", true);
...
int fih_ili_selftest_read(void){
	ipio_err("Ili_TestResult=%d, -1 return 1, 1 return 0\n",Ili_TestResult);
	if (Ili_TestResult==1)
	    return 0;
	else
	    return 1;
