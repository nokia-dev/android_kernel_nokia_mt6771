...
#define NVT_SHORT_TEST_CSV_FILE "/data/local/tmp/ShortTest.csv"
#define NVT_OPEN_TEST_CSV_FILE "/data/local/tmp/OpenTest.csv"
#define NVT_FW_RAWDATA_CSV_FILE "/data/local/tmp/FWMutualTest.csv"
#define NVT_FW_CC_CSV_FILE "/data/local/tmp/FWCCTest.csv"
#define NVT_NOISE_TEST_CSV_FILE "/data/local/tmp/NoiseTest.csv"
...
static int32_t nvt_tool_read_baseline(int32_t *xdata)
{
	uint8_t x_num = 0;
	uint8_t y_num = 0;
	uint32_t x = 0;
	...
	printk("%s:\n", __func__);
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, NVT_FW_RAWDATA_CSV_FILE, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
...
static int32_t nvt_tool_read_CC(int32_t *xdata)
{
	uint8_t x_num = 0;
	uint8_t y_num = 0;
	uint32_t x = 0;
	...
	if (ts->carrier_system) {
		printk("%s:RawData_CC_I:\n", __func__);
		// Save Rawdata to CSV file
		if (nvt_save_rawdata_to_csv(RawData_FW_CC_I, X_Channel, Y_Channel, NVT_FW_CC_CSV_FILE, 0) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
#if TOUCH_KEY_NUM > 0
		...
		printk("%s:RawData_CC_Q:\n", __func__);
		// Save Rawdata to CSV file
		if (nvt_save_rawdata_to_csv(RawData_FW_CC_Q, X_Channel, Y_Channel, NVT_FW_CC_CSV_FILE, rawdata_cc_q_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
	} else {
		// Save Rawdata to CSV file
		if (nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, NVT_FW_CC_CSV_FILE, 0) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
	}
...
static int32_t nvt_tool_read_fw_noise(int32_t *xdata)
{
	uint8_t x_num = 0;
	uint8_t y_num = 0;
	uint32_t x = 0;
	...
	printk("%s:RawData_Diff_Max:\n", __func__);
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(RawData_Diff_Max, X_Channel, Y_Channel, NVT_NOISE_TEST_CSV_FILE, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	...
	if (!ts->carrier_system) {
		...
		printk("%s:RawData_Diff_Min:\n", __func__);
		// Save Rawdata to CSV file
		if (nvt_save_rawdata_to_csv(RawData_Diff_Min, X_Channel, Y_Channel, NVT_NOISE_TEST_CSV_FILE, rawdata_diff_min_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
	}
...
static int32_t nvt_tool_read_fw_open(int32_t *xdata)
{
	uint32_t raw_pipe_addr = 0;
	uint8_t *rawdata_buf = NULL;
	uint32_t x = 0;
	...
	printk("%s:RawData_Open\n", __func__);
	// Save RawData to CSV file
	if (nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, NVT_OPEN_TEST_CSV_FILE, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
...
static int32_t nvt_tool_read_fw_short(int32_t *xdata)
{
	uint32_t raw_pipe_addr = 0;
	uint8_t *rawdata_buf = NULL;
	uint32_t x = 0;
	...
	if (ts->carrier_system)
		...
		printk("%s:RawData_Short\n", __func__);
	// Save Rawdata to CSV file
	if (nvt_save_rawdata_to_csv(xdata, X_Channel, Y_Channel, NVT_SHORT_TEST_CSV_FILE, 0) < 0) {
		NVT_ERR("save rawdata to CSV file failed\n");
		return -EAGAIN;
	}
	if (ts->carrier_system) {
		...
		printk("%s:RawData_Short_Base:\n", __func__);
		// Save Rawdata to CSV file
		if (nvt_save_rawdata_to_csv(RawData_Short_Base, X_Channel, Y_Channel, NVT_SHORT_TEST_CSV_FILE, rawdata_short_base_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
	}
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	//---FW Rawdata Test---
	if (nvt_tool_read_baseline(RawData_FWMutual) != 0) {
	...
	if (nvt_tool_read_CC(RawData_FW_CC) != 0) {
		TestResult_FW_CC = 1;
		if (ts->carrier_system) {
	...
	//---Noise Test---
	if (nvt_tool_read_fw_noise(RawData_Diff) != 0) {
	...
	//--Short Test---
	if (nvt_tool_read_fw_short(RawData_Short) != 0) {
	...
	//---Open Test---
	if (nvt_tool_read_fw_open(RawData_Open) != 0) {
...
int fih_nvt_selftest_open(void)
{
	...
	NVT_LOG("TestResult_Short TestResult = %d \n",TestResult_Short);
	NVT_LOG("TestResult_Short_Diff TestResult = %d \n",TestResult_Short_Diff);
	NVT_LOG("TestResult_Short_Base TestResult = %d \n",TestResult_Short_Base);
	NVT_LOG("TestResult_Open TestResult = %d \n",TestResult_Open);
	NVT_LOG("TestResult_FW_Rawdata TestResult = %d \n",TestResult_FW_Rawdata);
	NVT_LOG("TestResult_FWMutual TestResult = %d \n",TestResult_FWMutual);
	NVT_LOG("TestResult_FW_CC TestResult = %d \n",TestResult_FW_CC);
	NVT_LOG("TestResult_FW_CC_I TestResult = %d \n",TestResult_FW_CC_I);
	NVT_LOG("TestResult_FW_CC_Q TestResult = %d \n",TestResult_FW_CC_Q);
	NVT_LOG("TestResult_Noise TestResult = %d \n",TestResult_Noise);
	NVT_LOG("TestResult_FW_DiffMax TestResult = %d \n",TestResult_FW_DiffMax);
	NVT_LOG("TestResult_FW_DiffMin TestResult = %d \n",TestResult_FW_DiffMin);
	NVT_LOG("!!!!!!!!!TP selftest TestResult = %d !!!!!!!!!!!\n",TestResult_All);
