...
static ssize_t  flash_proc_write(struct file *file, const char *buffer, size_t count, loff_t *data)
{
	int  ret = 0;
	int  val = 0;
    char regBuf[2] = {'\0'};
	...
	return count;
	    ...
	    pr_info("%s: val(%d) number is wrong!!!\n", __func__, val);
...
static int mt6370_set_level_ch1(int level)
{
	...
	if (!flashlight_dev_ch1) {
		...
		printk("BBox; %s: Flashlight (led OVP) level failed\n", __func__);
		printk("BBox::UEC;%d::%d\n", 10, 1);
		return -1;
	}
