...
#ifndef FUNCTION_LONG_SHUTTER
#define FUNCTION_LONG_SHUTTER
#endif
...
extern void CAM_CAL_GetOtpData_4h7(void);
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
 ...
 { 3264, 2448,    272,    456, 2720, 1536, 1360,   768, 0000, 0000, 1360,  768,     0,  0, 1360,  768}, // hight speed video
...
static void write_shutter(kal_uint32 shutter)
{
	...
	#ifdef FUNCTION_LONG_SHUTTER
	kal_uint32 exp_time = 0;// shutter * 3688 * 1000 / 280000000;
	kal_uint32 value_coarse_2frame = 0;// reg_0x0202
	kal_uint32 value_coarse_1frame = 0;// reg_0x0340
	#endif
	printk("Enter! shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
	...
	// Update Shutter
#ifdef FUNCTION_LONG_SHUTTER
	/* add for slow shutter */
	exp_time = ((shutter * 3688) / 280000)+1;
	printk("exposure time %d\n",exp_time);
	
	value_coarse_2frame = (exp_time/1000) * 0x10B0;	// reg_0x0202
	value_coarse_1frame = value_coarse_2frame + 5;  // reg_0x0340
...
static void sensor_init(void)
{
	...
	// Base on S5K4H7_EVT0_Reference_setfile_v1.05
	write_cmos_sensor_8(0X0100, 0x00);
...
static void preview_setting(void)
{
	...
	write_cmos_sensor_8(0x030F, 0xA6);
	...
	write_cmos_sensor_8(0x0821, 0x98);
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor_8(0x0347, 0xD0);
	write_cmos_sensor_8(0x0348, 0x0B);
	write_cmos_sensor_8(0x0349, 0xB7);
	write_cmos_sensor_8(0x034A, 0x07);
	write_cmos_sensor_8(0x034B, 0xCF);
	write_cmos_sensor_8(0x034C, 0x05);
	write_cmos_sensor_8(0x034D, 0x50);
	write_cmos_sensor_8(0x034E, 0x03);
	write_cmos_sensor_8(0x034F, 0x00);
	write_cmos_sensor_8(0x0900, 0x01);
	write_cmos_sensor_8(0x0901, 0x22);
	write_cmos_sensor_8(0x0381, 0x01);
	write_cmos_sensor_8(0x0383, 0x01);
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor_8(0x030D, 0x06);
	write_cmos_sensor_8(0x030E, 0x00);
	write_cmos_sensor_8(0x030F, 0x6F);
	write_cmos_sensor_8(0x3C1F, 0x00);
	write_cmos_sensor_8(0x3C17, 0x00);
	write_cmos_sensor_8(0x3C1C, 0x05);
	write_cmos_sensor_8(0x3C1D, 0x15);
...
//	LOG_2;
  ...
  /*Get Front EEPROM Information*/
  CAM_CAL_GetOtpData_4h7();
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	MSDK_SENSOR_REG_INFO_STRUCT *sensor_reg_data=(MSDK_SENSOR_REG_INFO_STRUCT *) feature_para;
       /* if(!feature_para || !feature_para_len){
            printk("[MIMI]front feature_control null pointer feature_para:%p  feature_para_len:%p\n", feature_para,feature_para_len);
            return -EFAULT;
        }*/
	printk("feature_id = %d\n", feature_id);
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_FRAMERATE:
            //LOG_INF("current fps :%d\n", *feature_data);
                        printk("[MIMI@]current fps :%d\n", *feature_data_32);
			spin_lock(&imgsensor_drv_lock);
                        imgsensor.current_fps = *feature_data_32;//misty isp5.0 need to change
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_SET_HDR:
			//printk("[MIMI@]ihdr enable :%d\n", (BOOL)*feature_data_32);
			printk("[MIMI]Front Warning! Not Support IHDR Feature");
			spin_lock(&imgsensor_drv_lock);
