...
static imgsensor_info_struct imgsensor_info = {
	...
	.slim_video = {
		.pclk = 480000000,				/* record different mode's pclk */
		.linelength  = 4480,				/* record different mode's linelength */
		.framelength = 1189,			/* record different mode's framelength */
		.startx = 0,					/* record different mode's startx of grabwindow */
		.starty = 0,					/* record different mode's starty of grabwindow */
...
void four_seconds_mode(void)
{
	...
	write_cmos_sensor_8(0x3C17, 0x03);
	write_cmos_sensor_8(0x0820, 0x01);
	write_cmos_sensor_8(0x0821, 0x08);
	write_cmos_sensor_8(0x38C5, 0x06);
	write_cmos_sensor_8(0x38D9, 0x03);
	write_cmos_sensor_8(0x38DB, 0x03);
	write_cmos_sensor_8(0x38DD, 0x17);
	write_cmos_sensor_8(0x38C3, 0x08);
	write_cmos_sensor_8(0x38C1, 0x00);
	write_cmos_sensor_8(0x38D7, 0x13);
	write_cmos_sensor_8(0x38D5, 0x05);
	write_cmos_sensor_8(0x38B1, 0x02);
	write_cmos_sensor_8(0x3932, 0x18);
	write_cmos_sensor_8(0x3938, 0x20);
	write_cmos_sensor_8(0x0104, 0x01);  //20180628 coasia Lissy modify
	write_cmos_sensor_8(0x0340, 0x99);
	write_cmos_sensor_8(0x0341, 0x50);
	write_cmos_sensor_8(0x0342, 0x13);
	write_cmos_sensor_8(0x0343, 0x20);
	write_cmos_sensor_8(0x0202, 0x99);
	write_cmos_sensor_8(0x0203, 0x40);
	write_cmos_sensor_8(0x0104, 0x00);  //20180628 coasia Lissy modify
}
...
void two_seconds_mode(void)
{
	...
	write_cmos_sensor_8(0x3932, 0x18);
	write_cmos_sensor_8(0x3938, 0x20);
	write_cmos_sensor_8(0x0104, 0x01);  //20180628 Lissy modify
	write_cmos_sensor_8(0x0340, 0x4C);
	write_cmos_sensor_8(0x0341, 0xB0);
	write_cmos_sensor_8(0x0342, 0x13);
	write_cmos_sensor_8(0x0343, 0x20);
	write_cmos_sensor_8(0x0202, 0x4C);
...
static void sensor_init(void)
{
	...
	s5k3l6_table_write_cmos_sensor(
		...
		sizeof(addr_data_pair_init_s5k3l6) /
		sizeof(kal_uint16));
#else
...
#endif
	//write_cmos_sensor_8(0x3268, 0x00);/* affect test patten */
	//write_cmos_sensor(0x0100, 0x0000);
}	/*	sensor_init  */
...
static void preview_setting(void)
{
	...
	s5k3l6_table_write_cmos_sensor(
		...
		sizeof(addr_data_pair_preview_s5k3l6) /
		sizeof(kal_uint16));
#else
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_capture_s5k3l6[] = {
	0x0344, 0x0008,
	0x0346, 0x0008,
	0x0348, 0x1077,
	0x034A, 0x0C37,
...
#if MULTI_WRITE
	s5k3l6_table_write_cmos_sensor(
		addr_data_pair_capture_s5k3l6,
		sizeof(addr_data_pair_capture_s5k3l6) /
		sizeof(kal_uint16));
#else
	write_cmos_sensor(0x0344, 0x0008);
	write_cmos_sensor(0x0346, 0x0008);
...
static void hs_video_setting(void)
{
	...
	LOG_INF("E\n");
		...
		s5k3l6_table_write_cmos_sensor(
			...
			sizeof(addr_data_pair_hs_video_s5k3l6) /
			sizeof(kal_uint16));
#else
	...
	write_cmos_sensor(0x3C1E, 0x0000);
	write_cmos_sensor(0x030C, 0x0004);
	write_cmos_sensor(0x030E, 0x0076);
	write_cmos_sensor(0x3C16, 0x0002);
	write_cmos_sensor(0x0300, 0x0006);
	write_cmos_sensor(0x0342, 0x1320);
	write_cmos_sensor(0x0340, 0x0441);
...
static void slim_video_setting(void)
{
	...
	LOG_INF("slim_video_setting\n");
			...
			s5k3l6_table_write_cmos_sensor(
				...
				sizeof(addr_data_pair_slim_video_s5k3l6) /
				sizeof(kal_uint16));
#else
	...
	write_cmos_sensor(0x0300, 0x0006);
	write_cmos_sensor(0x0342, 0x1180);
	write_cmos_sensor(0x0340, 0x04A5);
	...
	write_cmos_sensor(0x380C, 0x003B);
	write_cmos_sensor(0x3064, 0xEFCF);
	write_cmos_sensor(0x309C, 0x0640);
	write_cmos_sensor(0x3090, 0x8000);
	write_cmos_sensor(0x3238, 0x000B);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	SET_PD_BLOCK_INFO_T *PDAFinfo;
       /* if(!feature_para || !feature_para_len){
            printk("[MIMI]main1 feature_control null pointer feature_para:%p  feature_para_len:%p\n", feature_para,feature_para_len);
            return -EFAULT;
        }*/
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_FRAMERATE:
			LOG_INF("[MIMI]current fps :%d\n", (UINT32)*feature_data_32);
			spin_lock(&imgsensor_drv_lock);
			imgsensor.current_fps = *feature_data_32;//misty isp5.0 need to change
			spin_unlock(&imgsensor_drv_lock);
			break;
		case SENSOR_FEATURE_SET_HDR:
			LOG_INF("[MIMI]ihdr enable :%d\n", (BOOL)*feature_data_32);
			spin_lock(&imgsensor_drv_lock);
			imgsensor.ihdr_en = (BOOL)*feature_data_32;//misty isp5.0 need to change
			spin_unlock(&imgsensor_drv_lock);
			break;
		...
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = rate;
		...
		case SENSOR_FEATURE_SET_PDAF:
			...
			imgsensor.pdaf_mode = (kal_uint16 )(*feature_data);
			break;
		#endif
