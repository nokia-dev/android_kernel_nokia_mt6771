...
static struct device_attribute tcpc_device_attributes[] = {
	...
	TCPC_DEVICE_ATTR(typec_cc_orientation, S_IRUGO),
};
...
enum {
	...
	TCPC_DESC_TIMER,
	TCPC_DESC_CAP_INFO,
	TCPC_DESC_TYPEC_CC_ORIENTATION,
};
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case TCPC_DESC_INFO:
		...
		if (tcpc->typec_local_rp_level == TYPEC_CC_RP_DFT)
			...
			i += snprintf(buf + i, 256, "rplvl = %s\n", "3.0");
		break;
	case TCPC_DESC_TYPEC_CC_ORIENTATION:
        if(tcpc->typec_remote_cc[0]>0)
		snprintf(buf, 256, "1\n");
        else if (tcpc->typec_remote_cc[1]>0)
		snprintf(buf, 256, "2\n");
        else
		snprintf(buf, 256, "0\n");
        break;
