...
#define APS_ERR(fmt, args...)    pr_debug(APS_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
static int alsps_irq_handler(void *data, uint len)
{
	...
	case SENSOR_HUB_NOTIFY:
		...
		case SCP_INIT_DONE:
			...
			break;
		case SCP_NOTIFY:
			if (rsp->notify_rsp.data[0] == EPL2182_NOTIFY_PROXIMITY_CHANGE) {
				gRawData.ps_state = rsp->notify_rsp.data[1];
				schedule_work(&obj->eint_work);
			} else if (rsp->notify_rsp.data[0] == EPL2182_NOTIFY_PROXIMITY_NOT_CHANGE) {
				gRawData.ps_state = rsp->notify_rsp.data[1];
...
{
	int idx, err = 0;
	int num = (int)(ARRAY_SIZE(epl2182_attr_list));
...
static int als_get_data(int *value, int *status)
{
	...
	if (atomic_read(&epl2182_obj->als_suspend) == 0) {
...
static int epl2182_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	epl2182_obj->als_level_num =
	    ARRAY_SIZE(epl2182_obj->hw.als_level);
	epl2182_obj->als_value_num =
	    ARRAY_SIZE(epl2182_obj->hw.als_value);
	WARN_ON(sizeof(epl2182_obj->als_level) != sizeof(epl2182_obj->hw.als_level));
	memcpy(epl2182_obj->als_level, epl2182_obj->hw.als_level, sizeof(epl2182_obj->als_level));
	WARN_ON(sizeof(epl2182_obj->als_value) != sizeof(epl2182_obj->hw.als_value));
