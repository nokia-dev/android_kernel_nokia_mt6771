...
#define aw87329_DRIVER_VERSION  "v1.0.0"
...
#define aw87329_CHIP_DISABLE    0x00
 ...
 ******************************************************************************/
#define BBOX_SPKAMP_ID_FAIL do {printk("BBox::UEC;50::0\n");} while (0);
#define BBOX_SPKAMP_PROBE_FAIL do {printk("BBox::UEC;50::1\n");} while (0);
#define BBOX_SPKAMP_READ_REGISTER_FAIL do {printk("BBox::UEC;50::2\n");} while (0);
#define BBOX_SPKAMP_WRITE_REGISTER_FAIL do {printk("BBox::UEC;50::3\n");} while (0);
#define BBOX_SPKAMP_POWERON_FAIL do {printk("BBox::UEC;50::4\n");} while (0);
#define BBOX_SPKAMP_POWEROFF_FAIL do {printk("BBox::UEC;50::5\n");} while (0);
...
unsigned char aw87329_audio_kspk_call(void);
unsigned char aw87329_audio_kspk_ringtone(void);



/*******************************************************************************
 * hac receiver amplifier functions
 ******************************************************************************/
unsigned char hac_hw_on(void);
unsigned char hac_hw_off(void);
...
struct aw87329_t{
    ...
    int reset_gpio;
    //fihtdc@ray: add hac enable gpio in speaker amp dirver ------ st.
    int hac_en_gpio;
    //fihtdc@ray: add hac enable gpio in speaker amp dirver ------ ed.
    unsigned char init_flag;
    unsigned char hwen_flag;
    unsigned char kspk_cfg_update_flag;
    unsigned char kspk_call_cfg_update_flag;
    unsigned char kspk_ringtone_cfg_update_flag;
    struct hrtimer cfg_timer;
    struct work_struct cfg_work;
};
struct aw87329_t *aw87329; 
...
struct aw87329_container *aw87329_kspk_call_cnt;
struct aw87329_container *aw87329_kspk_ringtone_cnt;

static char *aw87329_kspk_name = "aw87329_kspk.bin";
static char *aw87329_kspk_call_name = "aw87329_kspk_call.bin";
static char *aw87329_kspk_ringtone_name = "aw87329_kspk_ringtone.bin";

static unsigned char aw87329_kspk_cfg_default[] = {
0x39, 0x0E, 0xA3, 0x06, 0x05, 0x10, 0x07, 0x52, 0x06, 0x08, 0x96
};
static unsigned char aw87329_kspk_call_cfg_default[] = {
0x39, 0x0E, 0xA3, 0x06, 0x05, 0x10, 0x07, 0x52, 0x06, 0x08, 0x96
};
static unsigned char aw87329_kspk_ringtone_cfg_default[] = {
0x39, 0x0E, 0xA3, 0x06, 0x05, 0x10, 0x07, 0x52, 0x06, 0x08, 0x96
};
...
static int i2c_write_reg(unsigned char reg_addr, unsigned char reg_data)
{
    ...
    if (ret < 0)
    	 BBOX_SPKAMP_WRITE_REGISTER_FAIL
...
static unsigned char i2c_read_reg(unsigned char reg_addr)
{
    ...
    if (ret < 0)
    	 BBOX_SPKAMP_READ_REGISTER_FAIL
...
//fihtdc@ray: add hac enable gpio in speaker amp dirver --------------------------- st.
/*******************************************************************************
 * HAC amplifiercontrol
 ******************************************************************************/
unsigned char hac_hw_on(void)
{
    pr_info("%s enter\n", __func__);

    if (aw87329 && gpio_is_valid(aw87329->hac_en_gpio)) {
        gpio_set_value_cansleep(aw87329->hac_en_gpio, 0);
        msleep(2);
        gpio_set_value_cansleep(aw87329->hac_en_gpio, 1);
        msleep(2);
    } else {
        dev_err(&aw87329->i2c_client->dev, "%s:  failed\n", __func__);
    }
...
unsigned char aw87329_kspk_call_reg_val(unsigned char reg)
{
    if(aw87329->kspk_call_cfg_update_flag) {
        return *(aw87329_kspk_call_cnt->data+reg);
    } else {
        return aw87329_kspk_call_cfg_default[reg];
    }
}

unsigned char aw87329_kspk_ringtone_reg_val(unsigned char reg)
{
    if(aw87329->kspk_ringtone_cfg_update_flag) {
        return *(aw87329_kspk_ringtone_cnt->data+reg);
    } else {
        return aw87329_kspk_ringtone_cfg_default[reg];
    }
}
...
unsigned char aw87329_audio_kspk_call(void)
{
    ...
    i2c_write_reg(aw87329_REG_SYSCTRL , aw87329_kspk_call_reg_val(aw87329_REG_SYSCTRL )&0xF7);
    i2c_write_reg(aw87329_REG_MODECTRL, aw87329_kspk_call_reg_val(aw87329_REG_MODECTRL));
    i2c_write_reg(aw87329_REG_CPOVP   , aw87329_kspk_call_reg_val(aw87329_REG_CPOVP   ));
    i2c_write_reg(aw87329_REG_CPP     , aw87329_kspk_call_reg_val(aw87329_REG_CPP     ));
    i2c_write_reg(aw87329_REG_GAIN    , aw87329_kspk_call_reg_val(aw87329_REG_GAIN    ));
    i2c_write_reg(aw87329_REG_AGC3_PO , aw87329_kspk_call_reg_val(aw87329_REG_AGC3_PO ));
    i2c_write_reg(aw87329_REG_AGC3    , aw87329_kspk_call_reg_val(aw87329_REG_AGC3    ));
    i2c_write_reg(aw87329_REG_AGC2_PO , aw87329_kspk_call_reg_val(aw87329_REG_AGC2_PO ));
    i2c_write_reg(aw87329_REG_AGC2    , aw87329_kspk_call_reg_val(aw87329_REG_AGC2    ));
    i2c_write_reg(aw87329_REG_AGC1    , aw87329_kspk_call_reg_val(aw87329_REG_AGC1    ));
    i2c_write_reg(aw87329_REG_SYSCTRL , aw87329_kspk_call_reg_val(aw87329_REG_SYSCTRL ));
...
unsigned char aw87329_audio_kspk_ringtone(void)
{
    ...
    i2c_write_reg(aw87329_REG_SYSCTRL , aw87329_kspk_ringtone_reg_val(aw87329_REG_SYSCTRL )&0xF7);
    i2c_write_reg(aw87329_REG_MODECTRL, aw87329_kspk_ringtone_reg_val(aw87329_REG_MODECTRL));
    i2c_write_reg(aw87329_REG_CPOVP   , aw87329_kspk_ringtone_reg_val(aw87329_REG_CPOVP   ));
    i2c_write_reg(aw87329_REG_CPP     , aw87329_kspk_ringtone_reg_val(aw87329_REG_CPP     ));
    i2c_write_reg(aw87329_REG_GAIN    , aw87329_kspk_ringtone_reg_val(aw87329_REG_GAIN    ));
    i2c_write_reg(aw87329_REG_AGC3_PO , aw87329_kspk_ringtone_reg_val(aw87329_REG_AGC3_PO ));
    i2c_write_reg(aw87329_REG_AGC3    , aw87329_kspk_ringtone_reg_val(aw87329_REG_AGC3    ));
    i2c_write_reg(aw87329_REG_AGC2_PO , aw87329_kspk_ringtone_reg_val(aw87329_REG_AGC2_PO ));
    i2c_write_reg(aw87329_REG_AGC2    , aw87329_kspk_ringtone_reg_val(aw87329_REG_AGC2    ));
    i2c_write_reg(aw87329_REG_AGC1    , aw87329_kspk_ringtone_reg_val(aw87329_REG_AGC1    ));
    i2c_write_reg(aw87329_REG_SYSCTRL , aw87329_kspk_ringtone_reg_val(aw87329_REG_SYSCTRL ));
 ...
 ******************************************************************************/
static void aw87329_kspk_ringtone_cfg_loaded(const struct firmware *cont, void *context)
{
    unsigned int i;
    //int ret;

    if (!cont) {
        pr_err("%s: failed to read %s\n", __func__, aw87329_kspk_ringtone_name);
    ...
    aw87329_kspk_ringtone_cnt = kzalloc(cont->size+sizeof(int), GFP_KERNEL);
    if (!aw87329_kspk_ringtone_cnt) {
        release_firmware(cont);
        pr_err("%s: error allocating memory\n", __func__);
        return;
    }
    aw87329_kspk_ringtone_cnt->len = cont->size;
    memcpy(aw87329_kspk_ringtone_cnt->data, cont->data, cont->size);
    release_firmware(cont);

    for(i=0; i<aw87329_kspk_ringtone_cnt->len; i++) {
        pr_info("%s: rcv_cnt: addr:0x%02x, data:0x%02x\n",
                __func__, i, aw87329_kspk_ringtone_reg_val(i));
    }

    aw87329->kspk_ringtone_cfg_update_flag = 1;
}


static void aw87329_kspk_call_cfg_loaded(const struct firmware *cont, void *context)
{
    unsigned int i;
    int ret;

    if (!cont) {
        pr_err("%s: failed to read %s\n", __func__, aw87329_kspk_call_name);
        release_firmware(cont);
        ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG, aw87329_kspk_ringtone_name, 
                &aw87329->i2c_client->dev, GFP_KERNEL, NULL, aw87329_kspk_ringtone_cfg_loaded);
        if(ret) {
            aw87329->kspk_ringtone_cfg_update_flag = 0;    
    ...
    aw87329_kspk_call_cnt = kzalloc(cont->size+sizeof(int), GFP_KERNEL);
    if (!aw87329_kspk_call_cnt) {
        release_firmware(cont);
        pr_err("%s: error allocating memory\n", __func__);
        return;
    }
    aw87329_kspk_call_cnt->len = cont->size;
    memcpy(aw87329_kspk_call_cnt->data, cont->data, cont->size);
    release_firmware(cont);

    for(i=0; i<aw87329_kspk_call_cnt->len; i++) {
        pr_info("%s: rcv_cnt: addr:0x%02x, data:0x%02x\n",
                __func__, i, aw87329_kspk_call_reg_val(i));
    }

    aw87329->kspk_call_cfg_update_flag = 1;
...
static void aw87329_kspk_cfg_loaded(const struct firmware *cont, void *context)
{
    ...
    if (!cont) {
        ...
        release_firmware(cont);
        ret = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG, aw87329_kspk_call_name, 
                &aw87329->i2c_client->dev, GFP_KERNEL, NULL, aw87329_kspk_call_cfg_loaded);
        if(ret) {
            aw87329->kspk_call_cfg_update_flag = 0;
            pr_err("%s: request_firmware_nowait failed with read %s",
                    __func__, aw87329_kspk_call_name);
        }
        return;
...
static ssize_t aw87329_get_mode(struct device* cd,struct device_attribute *attr, char* buf)
{
    ...
    len += snprintf(buf+len, PAGE_SIZE-len, "2: kspk_call mode\n");
    len += snprintf(buf+len, PAGE_SIZE-len, "3: kspk_ringtone mode\n");
...
static int aw87329_parse_dt(struct device *dev, struct device_node *np) {
    ...
    //fihtdc@ray: add HAC gpio control in speaker amp driver
    aw87329->hac_en_gpio = of_get_named_gpio(np, "hac-en-gpio", 0);
    if (aw87329->hac_en_gpio < 0) {
        dev_err(dev, "%s: no hac enable gpio provided\n", __func__);
        return -1;
    } else {
        dev_info(dev, "%s: hac enable gpio provided ok\n", __func__);
    }
...
static int aw87329_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (gpio_is_valid(aw87329->hac_en_gpio)) {
        ret = devm_gpio_request_one(&client->dev, aw87329->hac_en_gpio,
            GPIOF_OUT_INIT_LOW, "hac_en");
        if (ret){
            dev_err(&client->dev, "%s: hac_en request failed\n", __func__);
            goto exit_gpio_request_failed;
        }
    }
